# Third-party Python dependency inventory

Generated with pip-licenses from the locked production CPU environment
(`uv sync --frozen --no-dev --extra cpu`). Model weights and operating-system
packages are outside this Python inventory; see `MODEL_LICENSES.md` and
`docs/open-source-compliance.md`.

| Package | Version | License | Project URL |
|---|---:|---|---|
| aliyun-python-sdk-core | 2.16.0 | Apache-2.0 | https://github.com/aliyun/aliyun-openapi-python-sdk |
| aliyun-python-sdk-kms | 2.16.5 | Apache-2.0 | http://develop.aliyun.com/sdk/python |
| annotated-doc | 0.0.5 | MIT | https://github.com/fastapi/annotated-doc |
| annotated-types | 0.8.0 | MIT | https://github.com/annotated-types/annotated-types |
| antlr4-python3-runtime | 4.9.3 | BSD | http://www.antlr.org |
| anyio | 4.14.2 | MIT | https://anyio.readthedocs.io/en/stable/versionhistory.html |
| audioread | 3.1.0 | MIT | https://github.com/beetbox/audioread |
| certifi | 2026.7.22 | MPL-2.0 | https://github.com/certifi/python-certifi |
| cffi | 2.1.0 | MIT-0 | https://cffi.readthedocs.io/en/latest/whatsnew.html |
| charset-normalizer | 3.4.9 | MIT | https://github.com/jawah/charset_normalizer/blob/master/CHANGELOG.md |
| click | 8.4.2 | BSD-3-Clause | https://github.com/pallets/click/ |
| colorama | 0.4.6 | BSD-3-Clause | https://github.com/tartley/colorama |
| crcmod | 1.7 | MIT | http://crcmod.sourceforge.net/ |
| cryptography | 49.0.0 | Apache-2.0 OR BSD-3-Clause | https://github.com/pyca/cryptography |
| decorator | 5.3.1 | BSD-2-Clause | UNKNOWN |
| fastapi | 0.141.1 | MIT | https://github.com/fastapi/fastapi |
| filelock | 3.32.2 | MIT | https://github.com/tox-dev/py-filelock |
| fsspec | 2026.7.0 | BSD-3-Clause | https://github.com/fsspec/filesystem_spec |
| funasr | 1.3.30 | MIT | https://github.com/modelscope/FunASR |
| h11 | 0.16.0 | MIT | https://github.com/python-hyper/h11 |
| hf-xet | 1.5.2 | Apache-2.0 | https://github.com/huggingface/xet-core |
| httpcore | 1.0.9 | BSD-3-Clause | https://www.encode.io/httpcore/ |
| httptools | 0.8.0 | MIT | https://github.com/MagicStack/httptools |
| httpx | 0.28.1 | BSD-3-Clause | https://github.com/encode/httpx |
| huggingface_hub | 1.25.1 | Apache-2.0 | https://github.com/huggingface/huggingface_hub |
| hydra-core | 1.3.4 | MIT | https://github.com/facebookresearch/hydra |
| idna | 3.18 | BSD-3-Clause | https://github.com/kjd/idna |
| jaconv | 0.5.0 | MIT | https://github.com/ikegami-yukino/jaconv |
| jamo | 0.4.1 | Apache-2.0 | https://github.com/jdongian/python-jamo |
| jieba | 0.42.1 | MIT | https://github.com/fxsjy/jieba |
| Jinja2 | 3.1.6 | BSD-3-Clause | https://github.com/pallets/jinja/ |
| jmespath | 0.10.0 | MIT | https://github.com/jmespath/jmespath.py |
| joblib | 1.5.3 | BSD-3-Clause | https://joblib.readthedocs.io |
| kaldiio | 2.18.1 | MIT | https://github.com/nttcslab-sp/kaldiio |
| lazy-loader | 0.5 | BSD-3-Clause | https://github.com/scientific-python/lazy-loader |
| librosa | 0.11.0 | ISC | https://librosa.org |
| llvmlite | 0.49.0 | BSD-2-Clause AND Apache-2.0 WITH LLVM-exception | http://llvmlite.readthedocs.io |
| markdown-it-py | 4.2.0 | MIT | https://github.com/executablebooks/markdown-it-py |
| MarkupSafe | 3.0.3 | BSD-3-Clause | https://github.com/pallets/markupsafe/ |
| mdurl | 0.1.2 | MIT | https://github.com/executablebooks/mdurl |
| modelscope | 1.39.0 | Apache-2.0 | https://github.com/modelscope/modelscope |
| modelscope-hub | 0.1.8 | Apache-2.0 | UNKNOWN |
| mpmath | 1.3.0 | BSD-3-Clause | http://mpmath.org/ |
| msgpack | 1.2.1 | Apache-2.0 | https://msgpack.org/ |
| narwhals | 2.24.0 | MIT | https://github.com/narwhals-dev/narwhals |
| networkx | 3.6.1 | BSD-3-Clause | https://networkx.org/ |
| numba | 0.67.0 | BSD-3-Clause | https://numba.pydata.org |
| numpy | 2.5.1 | BSD-3-Clause AND 0BSD AND MIT AND Zlib AND CC0-1.0 | https://numpy.org |
| omegaconf | 2.3.1 | BSD-3-Clause | https://github.com/omry/omegaconf |
| oss2 | 2.19.1 | MIT | http://oss.aliyun.com |
| packaging | 26.2 | Apache-2.0 OR BSD-2-Clause | https://github.com/pypa/packaging |
| platformdirs | 4.11.0 | MIT | https://github.com/tox-dev/platformdirs |
| pooch | 1.9.0 | BSD-3-Clause | https://www.fatiando.org/pooch/latest/changes.html |
| protobuf | 7.35.1 | BSD-3-Clause | https://developers.google.com/protocol-buffers/ |
| pycparser | 3.0 | BSD-3-Clause | https://github.com/eliben/pycparser |
| pycryptodome | 3.23.0 | BSD-3-Clause; Public domain | https://www.pycryptodome.org |
| pydantic | 2.13.4 | MIT | https://github.com/pydantic/pydantic |
| pydantic_core | 2.46.4 | MIT | https://github.com/pydantic |
| Pygments | 2.20.0 | BSD-2-Clause | https://pygments.org |
| pynndescent | 0.5.13 | BSD | http://github.com/lmcinnes/pynndescent |
| python-dotenv | 1.2.2 | BSD-3-Clause | https://github.com/theskumar/python-dotenv |
| python-multipart | 0.0.32 | Apache-2.0 | https://github.com/Kludex/python-multipart |
| PyYAML | 6.0.3 | MIT | https://pyyaml.org/ |
| RapidFuzz | 3.14.5 | MIT | https://github.com/rapidfuzz/RapidFuzz |
| regex | 2026.7.19 | Apache-2.0 AND CNRI-Python | https://github.com/mrabarnett/mrab-regex |
| requests | 2.34.2 | Apache-2.0 | https://github.com/psf/requests |
| rich | 15.0.0 | MIT | https://github.com/Textualize/rich |
| safetensors | 0.8.0 | Apache-2.0 | https://github.com/huggingface/safetensors |
| scikit-learn | 1.9.0 | BSD-3-Clause | https://scikit-learn.org |
| scipy | 1.18.0 | BSD-3-Clause | https://scipy.org/ |
| sentencepiece | 0.2.2 | Apache-2.0 | https://github.com/google/sentencepiece |
| shellingham | 1.5.4 | ISC | https://github.com/sarugaku/shellingham |
| six | 1.17.0 | MIT | https://github.com/benjaminp/six |
| soundfile | 0.14.0 | BSD-3-Clause | https://github.com/bastibe/python-soundfile |
| soxr | 1.1.0 | LGPL-2.1-or-later | https://github.com/dofuuz/python-soxr |
| starlette | 1.3.1 | BSD-3-Clause | https://github.com/Kludex/starlette |
| sympy | 1.14.0 | BSD-3-Clause | https://sympy.org |
| tensorboardX | 2.6.5 | MIT | https://github.com/lanpa/tensorboardX |
| threadpoolctl | 3.6.0 | BSD-3-Clause | https://github.com/joblib/threadpoolctl |
| tiktoken | 0.13.0 | MIT | https://github.com/openai/tiktoken |
| tokenizers | 0.22.2 | Apache-2.0 | https://github.com/huggingface/tokenizers |
| torch | 2.11.0+cpu | BSD-3-Clause | https://pytorch.org |
| torch-complex | 0.4.4 | Apache-2.0 | https://github.com/kamo-naoyuki/torch_complex |
| torchaudio | 2.11.0+cpu | BSD-3-Clause | https://github.com/pytorch/audio |
| tqdm | 4.70.0 | MPL-2.0 AND MIT | https://tqdm.github.io |
| transformers | 5.14.1 | Apache-2.0 | https://github.com/huggingface/transformers |
| typer | 0.27.0 | MIT | https://github.com/fastapi/typer |
| typing-inspection | 0.4.2 | MIT | https://github.com/pydantic/typing-inspection |
| typing_extensions | 4.16.0 | PSF-2.0 | https://github.com/python/typing_extensions |
| umap-learn | 0.5.12 | BSD | http://github.com/lmcinnes/umap |
| urllib3 | 2.7.0 | MIT | https://github.com/urllib3/urllib3/blob/main/CHANGES.rst |
| uvicorn | 0.52.0 | BSD-3-Clause | https://uvicorn.dev/ |
| watchfiles | 1.2.0 | MIT | https://github.com/samuelcolvin/watchfiles |
| websockets | 17.0 | BSD-3-Clause | https://github.com/python-websockets/websockets |

## Reviewed exceptions

- **kaldiio 2.18.1**: PyPI metadata reports UNKNOWN, but the installed distribution contains the upstream MIT LICENSE file. License SHA-256: `53c4d8a3763e37b835ac949a949570d0d7814f6f2e3a7a6d87e244e016fb9abd`.
- **soxr 1.1.0**: Required transitively by librosa; it is distributed as a separately replaceable Python extension and is not copied into Nota source. License SHA-256: `0dab6c9ba63620eecc17bcec0ebf03439b5b95a43a492bc3e701abb794b2d349`.
