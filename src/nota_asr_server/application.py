...
def create_app(
    ...
    app.state.manager_shutdown_token = None

    @app.post("/internal/manager/shutdown", include_in_schema=False)
    async def manager_shutdown(request: Request) -> JSONResponse:
        expected = request.app.state.manager_shutdown_token
        supplied = request.headers.get("X-Nota-Manager-Token")
